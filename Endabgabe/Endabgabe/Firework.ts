namespace Feuerwerk {
    ...
    export class Firework { //Firework-Klasse //hier haben wir unsere Attributen abgespeichert, um diese dann im Konstruktor zu bauen
        ...
        particles: Particle[]; // unser Partikel ist ein Array d.h: wir haben nicht nur ein Partikel, sondern viele verschiedene Partikel hintereinander 
...
// Konstruktor ist dazu da, unsere Klasse(Feuerwerk) zu bauen:
       ...
       // Mit constructor werden Instanzen geschaffen
        constructor(config: FireworkConfig, particleConfig: ParticleConfig) { //Firework und Particle müssen miteinander interagieren. Arbeiten zusammen
            
            let canvas: HTMLCanvasElement | null = document.querySelector("canvas");
            this.crc2 = <CanvasRenderingContext2D>canvas.getContext("2d"); //unser Feuerwerk wird in 2D dargestellt 
// Die Eigenschaften werden der Klasse zugewiesen/ / Sind Verpackung salopp gesagt auf denen Eigenschaften des Feuerwerks bzw. der Partikel stehen
            ...
            this.position = new Vector(config.positionX, config.positionY); //den Vektor unserer Position, wir holen über die Postionen X und Y automatisch gleich die Positionen raus. d.h dass er es gleich ausrechen kann
        ...
        draw(): void { //damit zeichnen wir jedesmal den Konstruktor  
        ...
        update(): void {
            ...
            for (let i: number = this.particles.length - 1; i >= 0; i--) { // hier holt er sich die Partikel Länge raus
