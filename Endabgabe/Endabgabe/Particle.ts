namespace Feuerwerk {
    ...
    //alles was wir hier einstellen, wird im Interface geladen 
    export interface ParticleConfig { // Ein Interface ist ein Typsystem, das eine beschreibende Liste von Eigenschaften bereitstellt, die ein Objekt besitzen muss.
        color: string; //Farbe meines einzelnen Partikels
        size: number; //Größe meines einzlenen Partikels
        width: number; // Ausbreitung der Partikel 
    }

    export class Particle { //Das ist meine Particle-Klasse //das sind die Partikel die rumfliegen.
        //diese Klasse enthält folgende Eigenschaften:
        alive: boolean; //ein boolescher Wert, der angibt, ob das Partikel aktiv ist oder nicht.
        lifetime: number = 20; //eine numerische Eigenschaft, die angibt, wie lange das Partikel existieren wird.
        position: Vector; //ein Vektor, der die x- und y-Position des Partikels auf dem Canvas speichert.
        velocity: Vector; //ein Vektor, der die Bewegungsrichtung und -geschwindigkeit des Partikels speichert.
        crc2: CanvasRenderingContext2D; //crc2 stellt also einen Zugriffspunkt auf diese Zeichenfläche dar, mit dessen Hilfe man darauf zeichnen kann.




        color: string; //es speichtert die Farbe des Partikels

        // Quasi das gewicht, bestimmt wie stark die Partikel nach unten gezogen werden. Sollte immer negativ sein.
        downForce: number; //eine numerische Eigenschaft, die das "Gewicht" des Partikels bestimmt und bestimmt, wie stark es nach unten gezogen wird.
        size: number; //eine numerische Eigenschaft, die die Größe des Partikels bestimmt.
        ...
        constructor(config: ParticleConfig, position: Vector, startVelocity: Vector) {
            ...
            let canvas: HTMLCanvasElement | null = document.querySelector("canvas"); // hier wird das HTML-Canvas-Element über die Methode "querySelector" gesucht und in eine Variable gespeichert
            this.crc2 = <CanvasRenderingContext2D>canvas.getContext("2d"); //Mit der Methode "getContext" wird dann aus dem Canvas ein 2D-Renderingkontext abgeleitet, der für die Darstellung der Partikel verwendet wird.
        }

        //Folgendes zu den Methoden (Funktioen): je nachdem welche Methode abgerufen wird, wird diese ausgeführt

        //Methode(Funktion): 
        update(): void { //Dieser Code stellt eine Methode dar, die verwendet wird, um die Position und Lebensdauer eines Partikels zu aktualisieren.
            ...
            if (this.lifetime <= 0) this.alive = false; // Wenn die Lebensdauer 0 oder weniger erreicht hat, wird das Partikel als "tot" markiert und kann gelöscht werden.
        }

        //Methode(Funktion): 
        draw(): void { //das draw zeichnet mir im Prinzip den Kreis der einzelen Partikeln (ein einzelner Kreis)
            if (!this.alive) return; //In dieser Zeile wird überprüft, ob die Eigenschaft "this.alive" auf "false" gesetzt ist. Wenn das der Fall ist, wird die Methode sofort mit "return" beendet, ohne weitere Aktionen auszuführen
